...
| FAN-004 | 加权温度绑定 | 多变量按权重计算有效温度 | P0 |
| FAN-005 | 零总权重拒绝 | 全部权重为 0 的绑定请求被拒绝并保持原配置 | P0 |
| FAN-006 | 部分变量失效 | 不可读变量被跳过，剩余正权重变量继续生效 | P1 |
| FAN-007 | 持久化恢复 | NVS/SD 卡恢复后仍保留有效加权绑定，非法零权重配置被忽略 | P1 |

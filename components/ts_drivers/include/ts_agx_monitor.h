/**
 * @file ts_agx_monitor.h
 * @brief AGX Device Monitor for TianShanOS
 * 
 * AGX 设备监控服务 - 通过 WebSocket 获取 Jetson 系统数据
 * 
 * 功能：
...
#ifndef TS_AGX_MONITOR_H
#define TS_AGX_MONITOR_H
...
#define TS_AGX_MONITOR_VERSION "1.0.0"
...
/** 最大 URL 长度 */
#define TS_AGX_MAX_URL_LEN          128

/** 最大错误消息长度 */
#define TS_AGX_MAX_ERROR_MSG_LEN    64

/** 最大时间戳长度 */
#define TS_AGX_MAX_TIMESTAMP_LEN    32

/** 最大 CPU 核心数 */
#define TS_AGX_MAX_CPU_CORES        16

/** 默认配置 */
#define TS_AGX_DEFAULT_SERVER_IP    "10.10.99.98"
#define TS_AGX_DEFAULT_SERVER_PORT  58090
#define TS_AGX_DEFAULT_RECONNECT_MS 3000
#define TS_AGX_DEFAULT_STARTUP_DELAY_MS 45000   // AGX 启动需要约 45 秒
#define TS_AGX_DEFAULT_HEARTBEAT_TIMEOUT_MS 10000
#define TS_AGX_DEFAULT_TASK_STACK   8192
#define TS_AGX_DEFAULT_TASK_PRIORITY 5
 ...
 */
typedef enum {
    TS_AGX_STATUS_UNINITIALIZED = 0,    /**< 未初始化 */
    TS_AGX_STATUS_INITIALIZED,          /**< 已初始化未启动 */
    TS_AGX_STATUS_CONNECTING,           /**< 连接中 */
    TS_AGX_STATUS_CONNECTED,            /**< 已连接 */
    TS_AGX_STATUS_DISCONNECTED,         /**< 已断开 */
    TS_AGX_STATUS_RECONNECTING,         /**< 重连中 */
    TS_AGX_STATUS_ERROR                 /**< 错误状态 */
} ts_agx_status_t;
 ...
 */
typedef struct {
    char timestamp[TS_AGX_MAX_TIMESTAMP_LEN];   /**< ISO 8601 时间戳 */
    
    /* CPU 信息 */
    struct {
        uint8_t core_count;                     /**< CPU 核心数 */
        ts_agx_cpu_core_t cores[TS_AGX_MAX_CPU_CORES];  /**< 各核心数据 */
    } cpu;
    
    /* 内存信息 */
    struct {
        ts_agx_memory_t ram;    /**< RAM 使用 */
        ts_agx_memory_t swap;   /**< SWAP 使用 */
    } memory;
    ...
    /* 功耗信息 */
    struct {
        ts_agx_power_t gpu_soc;     /**< GPU+SoC 功耗 */
        ts_agx_power_t cpu_cv;      /**< CPU 功耗 */
        ts_agx_power_t sys_5v;      /**< 系统 5V 功耗 */
    } power;
 ...
 */
typedef struct {
    char server_ip[TS_AGX_MAX_URL_LEN];     /**< 服务器 IP */
...
typedef struct {
    ...
    bool running;                           /**< 运行状态 */
    ts_agx_status_t connection_status;      /**< 连接状态 */
    ...
    char last_error[TS_AGX_MAX_ERROR_MSG_LEN]; /**< 最后错误消息 */
} ts_agx_status_info_t;

/**
 * @brief AGX 事件回调函数类型
 */
typedef void (*ts_agx_event_callback_t)(ts_agx_status_t status, 
                                        const ts_agx_data_t *data,
                                        void *user_data);
